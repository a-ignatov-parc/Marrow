...
// 
// * `0.3.3` - При запуске рантайма приложения в метод `init` вторым аргументом теперь передается 
...
// 
// * `0.3.2` - Сделан регрес в отношении создаваемой песочницы. Если использовать пустую страницу
// в качестве создаваемой песочницы, то невозможно будет такой лоадер встроить в чужую страницу 
// из-за ограничений кроссдоменой политики. Если в качестве урла вставить `javascript:0`, то 
// поломается транзит `transit.location` в Google Chrome из-за ограничений браузера на выставление 
// урла в страницу `about:blank`. Поэтому по умолчанию будет использоваться урл `javascript:0`, а 
// если потребуется полное функционирвоание `window.location` необходимо будет запустить 
// приложение с параметром `useLocationFix`, но приложение с таким параметром не будет работать 
// на кроссдоменной странице.
// 
