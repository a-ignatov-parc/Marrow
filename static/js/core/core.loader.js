...
// 
// * `1.2.16` - Исправлена работа инстанса загрузчика из приложения. Добавлена обработка случая, 
// когда путь до файла передан строкой, а не массивом. Так же исправлена ошибка, когда полностью 
// отсутсвует объект настроек.
// 
// * `1.2.15` - У классов добавлены ссылки на конструкторы + поправлена загрузка файлов ядра так, 
...
// 
// * `1.2.14` - В параметры объекта `hosts` добавлена ссылка на папку `html` и исправлена ссылка 
// открываемая в `iframe` на пустой `html` документ.
// 
// * `1.2.13` - Исправлена ошибка из-за которой не работал транзит урлов в Google Chrome.
// 
// * `1.2.12` - Исправлено "просачивание" метода `define` в основную область видимости у `curl.js`.
// 
// * `1.2.11` - Добавлен патч в составлении очереди на загрузку `js` файлов. Это увеличило время 
// инициализации приложения, но зато повысило отказоустойчивость инициализации.
// 
// * `1.2.10` - В режиме дебага выводим название используемого рецепта.
// 
// * `1.2.9` - Добавлено правило указывающее на папку `libs` в папке выбранного рецепта.
// 
// * `1.2.8` - Внесение изменений в загрузчик в связи с рефакторингом `core.js` + добавлено 
// правило для загрузчика указывающее на директорию с рецептом.
// 
// * `1.2.7` - Рефакторинг апи загрузчика.
// 
// * `1.2.6` - Загрузчик дописан для работы с рецептами приложений. Для обновления списка рецептов 
// необходимо запустить таски `grunt loader` или `grunt release`.
// 
// * `1.2.5` - В загрузку добавленны скомпелированные шаблоны приложения.
// 
// * `1.2.4` - Исправлена ошибка при повторной инициализации `curl.js`.
// 
// * `1.2.3` - Замена загрузчика `yepnope.js` на `curl.js` из-за необходимости загрузки 
// кросс-доменных файлов.
// 
// * `1.2.2` - Исправлена ошибка с определением момента готовности страницы для создания песочницы.
// 
// * `1.2.1` - Исправлена ошибка с использованием не правильной ссылки на `this.options`.
// 
// * `1.2.0` - Загрузчик переделан на полную загрузку приложения в песочницу с линкованием на 
// основной документ.
// 
...
window.WebApp.Loader.prototype = {
	...
	loadDependencies: function(window, webapp, callback) {
		...
		this.loader(filesList, callback || function() {}, true);
	},
	...
	initApp: function(App) {
		...
		// Добавляем префиксы и отправляем на загрузку
		this.loader(resources, complete, true);
	},
	...
	createLoader: function(window, sandbox) {
		...
		this.loader = (function(sandbox, global) {
			...
			return function(needs, callback, loadToSandbox) {
